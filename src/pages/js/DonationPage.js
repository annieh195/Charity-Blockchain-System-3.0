...
function DonationPage() {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        ...
        const { charityData, donationsData } = await fetchCharityAndDonations(charityName, adminKeys);
  ...
  const fetchCharityAndDonations = async (charityName, adminKeys) => {
    const res = await fetchTransactions(adminKeys.publicKey, "");
    ...
    const donationsData = res.getFilteredTransactions
      .filter(transaction => transaction.metadata.signerPublicKey !== adminKeys.publicKey)
      .map(transaction => ({
